export default class ThermostatUI {
  ...
  constructor(config) {
    ...
    root.appendChild(this._buildText(config.radius, 'prefixText', 0));
    ...
    root.appendChild(this._buildChevrons(config.radius, 0, 'low', 0.6, -config.radius / 2.5));
    root.appendChild(this._buildChevrons(config.radius, 0, 'high', 0.6, config.radius / 3));
    root.appendChild(this._buildChevrons(config.radius, 0, 'target', 1, 0));
    root.appendChild(this._buildChevrons(config.radius, 180, 'low', 0.6, -config.radius / 2.5));
    root.appendChild(this._buildChevrons(config.radius, 180, 'high', 0.6, config.radius / 3));
  ...
  updateState(options,hass) {
    ...
    this.hvac_fan_modes = options.entity.attributes.fan_modes;
    this.temperature = {
    ...
    this._updateTicks(from, to, tick_indexes, ambient_index);
  ...
  updatePrefixText(entity) {
    ...
    switch(hvac_action) {
        case 'idle':
            prefixText = this.getIdlePrefixText(entity.state);
            break;
        case 'heating':
            prefixText = "HEATING";
            break;
  ...
  _updateTicks(from, to, large_ticks, ambient) {
    const config = this._config;
    ...
    const tickPointsTarget = [
      [config.radius - 2.5, config.ticks_outer_radius],
      [config.radius + 2.5, config.ticks_outer_radius],
      [config.radius + 2.5, config.ticks_inner_radius + 20],
      [config.radius - 2.5, config.ticks_inner_radius + 20]
    ];

    const tickPointsCurrent = [
        [config.radius - 2.5, config.ticks_outer_radius],
        [config.radius + 2.5, config.ticks_outer_radius],
        [config.radius + 2.5, config.ticks_inner_radius],
        [config.radius - 2.5, config.ticks_inner_radius]
      ];
    ...
    this._ticks.forEach((tick, index) => {
      let isPoint = false;
      let isTarget = false;
      let tickClass = (index >= from && index <= to) ? 'active' : '';
      ...
      if (isPoint) tickClass += ' large';
      if (isTarget) tickClass += ' isTarget';
  ...
  _updateDialog(modes, fan_modes, hass){
    ...
    for(var i=0;i<modes.length;i++){
      ...
      this.createMode(mode, icon, hass);
    }
    if(fan_modes && fan_modes.length > 0) {
        this.createMode("fan", "fan", hass);
    }
  }

  createMode(mode, icon, hass) {
    let d = document.createElement('span');
    d.innerHTML = `<ha-icon class="modeicon ${mode}" icon="mdi:${icon}"><span>${mode}</span></ha-icon>`
  ...
  _buildText(radius, name, offset) {
    const target = SvgUtil.createSVGElement('text', {
      x: radius + offset,
      y: radius-(name=='prefixText'?radius/2 - 35:0),
